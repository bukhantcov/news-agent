...
DB_FILE = "gigachat_html_published.json"
...
def send_to_telegram(text, parse_mode='HTML'):
    """Отправка сообщения с поддержкой HTML"""
...
def format_html_post(title, source, summary, numbers, url=None):
    """Форматирование поста в HTML для Telegram"""
    ...
    title_clean = escape_html(title)
    source_clean = escape_html(source)
    summary_clean = escape_html(summary)
    numbers_clean = escape_html(numbers)
    
    # HTML-шаблон для Telegram
    post = f"""⚡️ <b>{title_clean}</b>

📅 <b>Источник:</b> {source_clean}

<blockquote>{summary_clean}</blockquote>
"""

    if numbers_clean and numbers_clean != 'Нет':
        post += f"\n📊 <b>Ключевые цифры:</b>\n<code>{numbers_clean}</code>\n"
...
Найди в интернете 3 самые важные НОВОСТИ из мира ИИ (искусственный интеллект) за последние 6 часов.

Требования к новости:
1. Это должно быть КОНКРЕТНОЕ СОБЫТИЕ (релиз, анонс, обновление модели, партнерство)
2. Должны быть КОНКРЕТНЫЕ КОМПАНИИ (OpenAI, Google, Microsoft, Meta, Anthropic, NVIDIA)
3. Должны быть ЦИФРЫ (проценты, деньги, количество параметров)
4. НЕ публикуй обзоры, рейтинги, дайджесты
...
Если за последние 6 часов нет новостей — напиши "НЕТ НОВОСТЕЙ"."""
...
def main():
    print("="*70)
    print("🤖 GIGACHAT HTML AGENT")
    print("📡 Поиск новостей через ИИ с доступом в интернет")
    ...
    while True:
        ...
        if not access_token:
            print("⏳ Не удалось получить токен. Жду 30 минут...")
            time.sleep(30*60)
            continue
        ...
        if not response:
            print("⏳ GigaChat не ответил. Жду 30 минут...")
            time.sleep(30*60)
            continue
        ...
        if not news_list:
            print("⏳ Нет новостей за последние 6 часов. Жду 30 минут...")
            time.sleep(30*60)
            continue

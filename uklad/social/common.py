"""
Общий транспорт для постинга в Threads и Instagram через Playwright.
...
login_instagram.py): пароль этот код не видит и не хранит, сохраняется только
storage_state сессии (cookies), локально, вне git.
"""
...
logger = logging.getLogger("uklad_social")
...
class PostSession:
    ...
    def __init__(self, session_path: Path, headless: bool = True):
        if not session_path.exists():
            raise FileNotFoundError(
                f"Нет сессии {session_path}. Сначала запустите соответствующий login_*.py"
            )
    ...
    async def __aenter__(self) -> "PostSession":
        ...
        context = await self._browser.new_context(
            ...
            viewport={"width": 430, "height": 932},  # мобильная раскладка — у обеих площадок композер иначе на десктопе
        )

...
def process_with_gigachat(news_list, access_token):
    ...
    # Формируем данные
    news_text = ""
    for i, news in enumerate(news_list[:1], 1):  # Берем только 1 новость для простоты
        news_text += f"""
...
Ссылка: ..."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return content, news_list[0]['url'], news_list[0]['source']
    except Exception as e:
        print(f"   ❌ Ошибка: {e}")
    return None, None, None

def parse_news(text, fallback_url, fallback_source):
    if not text:
        return None
    ...
    if title_match:
        ...
        news['title'] = "Новости искусственного интеллекта"
    ...
    # Всегда используем оригинальную ссылку
...
def main():
    print("="*60)
    print("🤖 NEWS AGENT V4.2 — ПОЛНЫЕ НОВОСТИ СО ССЫЛКОЙ")
    ...
    response, original_url, original_source = process_with_gigachat(news_list, token)
    if not response:
        print("❌ GigaChat не ответил")
        return 1
    
    news = parse_news(response, original_url, original_source)
    if not news:

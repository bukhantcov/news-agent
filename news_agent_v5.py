...
def is_published(url, title):
    ...
    if url in db['urls']:
        return True
    # Проверяем по заголовку (нормализованному)
...
def mark_published(url, title):
    ...
    if url not in db['urls']:
        db['urls'].append(url)
    # Сохраняем нормализованный заголовок
    normalized_title = title.lower().replace(' ', '').replace('-', '')[:50]
...
def process_with_gigachat(news_list, access_token):
    ...
    news = news_list[0]  # Берем первую новость
...
def main():
    ...
    if not is_working_hours():
        print(f"⏰ Не рабочее время. Сейчас {datetime.now().strftime('%H:%M')}, работаем с {WORK_START_HOUR}:00 до {WORK_END_HOUR}:00")
        return 0
    
    print("="*60)
    print("🤖 NEWS AGENT V5 — РАБОЧЕЕ ВРЕМЯ + ЗАЩИТА ОТ ДУБЛЕЙ")
    ...
    # Проверяем первую новость на дубль
